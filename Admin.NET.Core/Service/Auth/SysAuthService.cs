...
public class SysAuthService : IDynamicApiController, ITransient
{
    ...
    public async Task<LoginOutput> Login([Required] LoginInput input)
    {
        ...
        return await CreateToken(user, tenant);
    }
    ...
    /// <returns></returns>
    [NonAction]
    public async Task<LoginOutput> CreateToken(SysUser user, SysTenant tenant = null)
    {
        // 单用户登录
        ...
        if (tenant != null)
        {
            loginOutput.TenantExpiration = tenant.Expiration;
            loginOutput.TenantStatus = tenant.Status;
        }
